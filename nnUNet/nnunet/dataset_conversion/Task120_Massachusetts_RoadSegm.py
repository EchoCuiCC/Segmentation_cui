...
if __name__ == '__main__':
    ...
    base = '/root/workspace/data/DATASET/DRIVE'
    # this folder should have the training and testing subfolders

    # now start the conversion to nnU-Net:
    task_name = 'Task003_DRIVE'
    ...
    for t in training_cases:
        unique_name = t[:2]  # just the filename with the extension cropped away, so img-2.png becomes img-2 as unique_name
        print(unique_name)
        input_segmentation_file = join(labels_dir_tr, t)
        input_image_file = join(images_dir_tr, unique_name+'_training.tif')
    ...
    for ts in testing_cases:
        ...
        input_image_file = join(images_dir_ts, unique_name+'_training.tif')
    ...
    generate_dataset_json(join(target_base, 'dataset.json'), target_imagesTr, target_imagesTs, ('X'),
                          labels={0:'background',1: 'target'}, dataset_name=task_name, license='hands off!')
