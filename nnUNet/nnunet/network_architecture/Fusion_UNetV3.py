...
# Generic_Unet 通用Unet
class Fusion_UNetV3(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 seg_output_use_bias=False,centerline=False,branch_block_num=0,fusion_num=0,splitUp=False,alpha=1):
        """
        ...
        """
        super(Fusion_UNetV3, self).__init__()

        # 修改网络层数
        assert branch_block_num<=num_pool,'分支数超过了下采样层数'
        assert fusion_num<=branch_block_num,"tcl融合模块数超过了分支数目"
        ...
        self.branch_block_num = branch_block_num
        ...
        for u in range(num_pool):
            ...
            if not self.convolutional_upsampling:
                ...
                if centerline and u>=num_pool-branch_block_num and self.splitUp :
            ...
            if centerline and u>=num_pool-branch_block_num:
                if u>=num_pool-fusion_num:
                    print(u)
                    self.TCL.append(nn.Sequential(
                        TCLblock(nfeatures_from_skip,nfeatures_from_skip,
                        self.conv_op,self.norm_op,self.norm_op_kwargs,self.dropout_op,
                        self.nonlin,self.nonlin_kwargs)
                    ))
    ...
    def forward(self, x):
        ...
        gap = self.num_pool-self.branch_block_num
        ...
        for u in range(len(self.tu)):
            ...
            if self.centerline and u>=self.num_pool-self.branch_block_num:
                # print(x.shape,y.shape,u-gap)
                x = self.tu[u](x)
                y = self.tu_cen[u-gap](y)
                # print('after tu, ',x.shape,y.shape)
                
                if u>=self.num_pool-self.fusion_num:
                    public_information = self.TCL[u-self.num_pool+self.fusion_num](x+y)

                    x = self.alpha*x+(1-self.alpha)*public_information
                    y = self.alpha*y+(1-self.alpha)*public_information
