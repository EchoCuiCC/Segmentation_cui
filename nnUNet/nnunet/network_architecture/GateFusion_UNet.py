...
class GCT(nn.Module):
    ...
    def forward(self, x):
        ...
        gate = torch.softmax(embedding.view([2,shape[0],int(self.num_channels/2),1,1,1]),0)
        return gate
...
# Generic_Unet 通用Unet
class GateFusion_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        """
        super(GateFusion_UNet, self).__init__()
        ...
        for u in range(num_pool):
            ...
            if centerline and u>=num_pool-tcl_block_num:
                ...
                self.gateX.append(nn.Sequential(
                    GCT(nfeatures_from_skip*2)
                ))
                # self.gateY.append(nn.Sequential(
                #     GCT(nfeatures_from_skip*2)
                # ))
        ...
        if centerline:
            ...
            self.gateX = nn.ModuleList(self.gateX)
            # self.gateY = nn.ModuleList(self.gateY)
        if self.upscale_logits:
    ...
    def forward(self, x):
        ...
        for u in range(len(self.tu)):
            ...
            if self.centerline and u>=self.num_pool-self.tcl_block_num:
                ...
                gateX = self.gateX[u-gap](torch.cat((public_information, x), dim=1))
                # gateY = self.gateY[u-gap](torch.cat((public_information, y), dim=1))
                x = gateX[0]*public_information+gateX[1]*x
                # y = gateY[0]*public_information+gateY[1]*y
