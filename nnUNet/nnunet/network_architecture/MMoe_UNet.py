...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 seg_output_use_bias=False,num_experts=0,input_size=None):
        """
        ...
        self.input_size = np.array(input_size, dtype=int).prod()
        self.task_name = ['seg','centerline']
        self.gate_specific = nn.ModuleDict({task: nn.Sequential(nn.Linear(input_size, num_experts),
        ...
        for _ in range(num_experts):
            ...
            self.encoder_list.append(conv_blocks_context)
        ...
        for ds in range(len(self.conv_blocks_localization)):
            ...
            self.cen_outputs.append(conv_op(self.conv_blocks_localization[ds][-1].output_channels, num_classes,
                                            1, 1, 0, 1, 1, seg_output_use_bias))
        ...
        self.cen_outputs = nn.ModuleList(self.cen_outputs)
        if self.upscale_logits:
    ...
    def forward(self, x):
        ...
        selector_seg = self.gate_specific['seg'](torch.flatten(x, start_dim=1)) 
        selector_cen = self.gate_specific['cen'](torch.flatten(x, start_dim=1)) 
        for expert in range(self.num_experts):
            conv_blocks_context = self.experts[expert]
            seg_outputs = []
            cen_outputs = []
            ...
            experts_shared_rep.append(x)
        experts_shared_rep = torch.stack(experts_shared_rep)

        gate_rep_seg = torch.einsum('ij..., ji -> j...', experts_shared_rep, selector_seg)
        gate_rep_seg.requires_grad = True
        seg_outputs.append(self.final_nonlin(self.seg_outputs[u](gate_rep_seg)))

        gate_rep_cen = torch.einsum('ij..., ji -> j...', experts_shared_rep, selector_cen)
        gate_rep_cen.requires_grad = True
        out_cen = self.final_nonlin(self.seg_outputs[u](gate_rep_cen))            
        ...
        if self._deep_supervision and self.do_ds:                   
            ...
            centerline = tuple([cen_outputs[-1]]+[i(j) for i, j in zip(list(self.upscale_logits_ops)[::-1], cen_outputs[:-1][::-1])])
