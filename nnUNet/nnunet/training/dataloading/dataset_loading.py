...
def load_dataset(folder, num_cases_properties_loading_threshold=1000):
    ...
    # Tag:trainer v2的dataset，不用专门写类
    # 这里的dataset只是一个路径选择，dataset是个字典，第一层关键字是病例，
    # 每个病例进入的关键字是数据路径'data_file',数据属性路径'properties_file',数据属性'properties'
    # properties由一个关键字是crop_bbox，这里其实数据已经保留过了,因为artery需要先去掉部分无用铁板信息
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        这里的意思是说，这个dataloader打开的数据集，最好是npy格式的，虽然这里默认是npz格式，因为它做实验用的是decathlon dataset，这个数据集非常的大。
        如果全部都解压缩乘npy的话，数据会超过1t放不下，所以这里才采用一边解压一边拿进来，如果数据不大的，就直接先解压缩成npy就可以了。
        ...
        oversample_foreground 这个比例的batch会强制包含一些前景像素
        """
        ...
        # self._data是父类里中构造的，其中的内容就是self._data=data
    ...
    def get_do_oversample(self, batch_idx):
        # batch_id如果>=batch_size的前0.7就会做oversample
    ...
    def generate_train_batch(self):
        # TAG:batchgenerator的dataloader的生产函数，要用这个来得到数据
        # 这个必须要重载的
        ...
        for j, i in enumerate(selected_keys):
            ...
            # case_all_data的shape是 2,d,w,h 2是因为ct只有一个图片，它这里0是图片，通道1是target
            ...
            # need_to_pad就是增强patch_size-网络patch_size
            # 这里的patch_size是裁剪的patchsize,输入网络的叫final_patch
            ...
            if not force_fg:
                ...
                # 求前景像素的类别数据,并且要除开为负数的那些类别，也就是说必须保留的数据可以自行选择
                # 这里在下方selected_class是随机的，如果必须保留某一类数据也可以
        ...
        # 这里返回的数据data的结构是bcdwh,properties是里面包含的两个patch的数据源信息，keys是他俩的名字
