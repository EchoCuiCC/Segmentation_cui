...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.in_epoch_train = True

        # tag:trainerV2将deterministic=False，即不需要确定性训练，但benchmark=Ture加速训练
        if deterministic:
        ...
        self.all_tr_seg_dice = []
        self.all_val_seg_dice = []
        self.all_tr_dc_dice = []
        self.all_val_dc_dice = []
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        save_this = {
            ...
            'lr_scheduler_state_dict': lr_sched_state_dct,
             # -- 【DC】记录了centerline中间dc的写法   
            'plot_stuff': (self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode,
                           self.all_val_eval_metrics,self.all_tr_dc_dice,self.all_val_dc_dice,self.all_val_seg_dice,self.all_tr_seg_dice),
             # -- 记录了centerline中间dc的写法   

            # # -- 不记录了centerline中间dc的写法  
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        # # --不记录centerline中间训练的原本的写法
        # self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode, self.all_val_eval_metrics = checkpoint[
        #     'plot_stuff']
        # # --不记录centerline中间训练的原本的写法

        # ---【DC】记录了centerline中间dc的写法
        self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode, self.all_val_eval_metrics,self.all_tr_dc_dice,self.all_val_dc_dice,self.all_val_seg_dice,self.all_tr_seg_dice= checkpoint[
            'plot_stuff']
        # ---记录了centerline中间dc的写法
    ...
    def run_training(self):
        ...
        # 是否使用混合精度训练
        # tag:混合精度运算，trainerV2中将fp32=False，即采用混合精度训练
        # 使用混合精度运算与否取决于trainner中fp16变量，如果为True，则使用，也就是说如果要用这个将self.fp16=True即可
        self._maybe_init_amp()
        ...
        if cudnn.benchmark and cudnn.deterministic:
            warn("torch.backends.cudnn.deterministic is True indicating a deterministic training is desired. "
                ...
                #  "torch.backends.cudnn.deterministic is True 表示需要确定性训练。" "
                # 但是 torch.backends.cudnn.benchmark 也是 True，这将阻止确定性训练！" "
                # 如果你想要确定性，那么设置 benchmark=False
                # benchmark可以加速训练
                # deterministic表示确定性训练
        ...
        while self.epoch < self.max_num_epochs:
            ...
            # use_progress_bar默认是false，即不显示bar条
            # 这个显示的话,样式像这样
            # Epoch 1/30: 100%|██████████| 5/5 [00:00<00:00, 200.59it/s, loss=4]
            # train:train部分
            if self.daw:
                self.train_losses_epoch_tasks = []
                try:
                    print('[seg,centerline]  -1 loss:{}, -2 loss:{}'.format(self.train_losses_tasks[-1],self.train_losses_tasks[-2]),)
                    weight_l = (self.train_losses_tasks[-1]+1e-3)/(self.train_losses_tasks[-2]+1e-3)
                except:
                    weight_l = [1,1]
                finally:
                    self.weight = torch.softmax(torch.Tensor(weight_l),0)          
                self.print_to_log_file(' daw weight: ',self.weight)  
            ...
            if self.use_progress_bar:
                ...
                for _ in range(self.num_batches_per_epoch):
                    ...
                    # ---【DC】如果选用了centerline将会记录下训练中间 seg dice和dc dice的变化
                    if self.centerline:
                        l,seg,dc = self.run_iteration(self.tr_gen, True)
                        train_losses_epoch.append(l)
                        train_seg_dice.append(seg)
                        train_dc_dice.append(dc)
                    else:
                        l = self.run_iteration(self.tr_gen, True)
                        train_losses_epoch.append(l)

            if self.daw:
                self.train_losses_tasks.append(np.mean(self.train_losses_epoch_tasks,axis=0))
            if self.centerline:
                self.all_tr_seg_dice.append(np.mean(train_seg_dice))
                self.all_tr_dc_dice.append(np.mean(train_dc_dice))
                self.print_to_log_file("train seg dice: %.4f" % self.all_tr_seg_dice[-1])
                self.print_to_log_file("train dc dice: %.4f" % self.all_tr_dc_dice[-1])
            self.all_tr_losses.append(np.mean(train_losses_epoch))
            self.print_to_log_file("train loss : %.4f" % self.all_tr_losses[-1])


            # train:val部分
            self.in_epoch_train=False
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    if self.centerline:
                        # -- 【DC】记录了centerline中间dc的写法                 
                        l,seg,dc = self.run_iteration(self.val_gen, False, True)
                        val_losses.append(l)
                        val_seg_dice.append(seg)
                        val_dc_dice.append(dc)
                    else:
                        l = self.run_iteration(self.val_gen, False, True)
                if self.centerline:        
                    self.all_val_seg_dice.append(np.mean(val_seg_dice))
                    self.all_val_dc_dice.append(np.mean(val_dc_dice))
                    self.print_to_log_file("validation seg dice: %.4f" % self.all_val_seg_dice[-1])
                    self.print_to_log_file("validation dc dice: %.4f" % self.all_val_dc_dice[-1])
